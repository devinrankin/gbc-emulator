...
uint8_t sm83_read_r8_or_hl(sm83_t* sm83, uint8_t index) {
    switch(index) {
        ...
        case 6: return bus_read8(sm83->cartridge, sm83_read_r16(sm83, SM83_REGISTER_PAIR_HL));
...
void sm83_write_r8_or_hl(sm83_t* sm83, uint8_t index, uint8_t val) {
    switch(index) {
        ...
        case 6: bus_write8(sm83->cartridge, sm83_read_r16(sm83, SM83_REGISTER_PAIR_HL), val); break;
        default:
            NO_IMPL
    }
}
...
/* Fetches the next byte in ROM and increments the PC */
static uint8_t sm83_fetch8(sm83_t* sm83) {
    uint8_t value = bus_read8(sm83->cartridge, sm83->registers.pc);
    sm83->registers.pc++;
